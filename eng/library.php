		...
		<div id="content">
			<?
				if($id==0)
				{
					...
					echo
					'
						<h1>Bachelor projects of Saint-Petersburg State Polytechnical University</h1>
							...
							if($book)
								do
								{
									...
									<tr class="table">
										...
										<td class="bookdescr">
											<h2>
												<a href="book.php?id=%s">%s</a>
											</h2>
											<p>%s (<a href="book.php?id=%s">More</a>)</p>
										</td>
									</tr>
									', $book["id"], $book["id"], $book["name_en"], $book["id"], $book["name_en"], $book["mini_descr_en"], $book["id"]);
								}
								while($book=mysql_fetch_array($result));
							else
								echo"<p>There are no books here now.</p>";
						?>
					</table>
					<?
						if($_SESSION['priv']==1)
							echo'<br><a class="add" href="add_book.php">Add a book</a>';
				}
					?>
			<br>
